...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        d = d + random.gauss(0.0,self.distance_noise)
        ...
        if abs(beta) >= 0.001:
            ...
            thetap = (theta +  beta) % (2.0 * pi)
            xp = cx + (sin(theta + beta) * r)
            yp = cy - (cos(theta + beta) * r)

        else:
            xp = self.x + (d * cos(theta))
            yp = self.y + (d * sin(theta))
            thetap = (theta + beta) % (2.0 * pi)
    ...
    # sense: 
    #    
    def sense(self, exact): #do not change the name of this function
        Z = []
        ...
        for i in range(len(landmarks)):
            ...
            theta=atan2(landmark[0]-self.y,landmark[1]-self.x)-self.orientation 
            if theta<0:
                theta %= 2*pi
            Z.append(theta)        
