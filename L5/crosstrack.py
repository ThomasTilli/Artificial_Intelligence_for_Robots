...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        ...
        #
        #    
        (x,y) = (self.x-2*radius,self.y-radius)   # shift to symmetry
        if(abs(x)>radius): # on a curve
            return hypot(abs(x)-radius,y)-radius
        else: # on a straightaway
            return copysign(1,cos(self.orientation))*y-radius        
        return cte
